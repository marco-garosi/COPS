...
class Grid2Image(nn.Module):
    ...
    def forward(self, x, nnbatch, zz_int, yy, xx):
        ...
        point_depth = point_depth.view(-1, xx.shape[-1])
...
class Realistic_Projection:
    ...
    def get_img(self, points):
        ...
        mappings = -1 * torch.ones((self.num_views, params[net]['resolution'], params[net]['resolution']), dtype=torch.int).to(self.device)
        point_in_view = torch.repeat_interleave(torch.arange(0, self.num_views)[:, None], 2048).view(-1, ).to(self.device)
        yy = point_loc_in_img[:, :, 0].view(-1).long()
        xx = point_loc_in_img[:, :, 1].view(-1).long()
        # is_seen[is_seen == 0] = -1
        # mappings[point_in_view, xx, yy] = is_seen.view(-1).int()
